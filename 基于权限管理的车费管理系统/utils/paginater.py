class Paginater():
    ...
    def __init__(self, url_address, cur_page_num, total_rows, one_page_lines=10, page_maxtag=9):
        """
        :param url_address: 需要分页的功能的网页url
        :param cur_page_num: 当前页码数
        :param total_rows: 模型数据的记录总数
        ...
        # 把当前页码标签放在中间，前面放一半页码标签，后面也放一半页码标签
        # 因此先把页面上设置的页面标签数除以2
        ...
        """
        如果计算出的页码标签开始数小于1，页码中页码标签设置为从1开始
    ...
    def html_page(self):
        ...
        if self.cur_page_num <=1:
            html_page.append('<li class="disabled"><a href="#"><span aria-hidden="true">&laquo;</span></a></li>'.format(self.cur_page_num - 1))
        else:
            # 上一页页码标签的html代码
